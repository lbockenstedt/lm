...
class SourceHub:
    ...
    def __init__(self, base_url, insecure=True):
    ...
    def snapshot(self):
        """One fleet snapshot: clients + VMs, as the WebUI's own dashboard reads
        them. Missing endpoints degrade to empty rather than aborting — a source
        hub on an older branch may not serve all of them."""
        out = {}
