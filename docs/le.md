...
## AppBuilder identity cert

The ab agent authenticates to the hub's reverse `HUB_REQUEST` channel (log reads + fleet-update triggers) over mutual TLS, presenting an LE-issued, SAN-pinned cert. The cert is produced and distributed through the normal LE flow and then *pinned* by domain. See [ab.md](ab.md) for the AppBuilder agent/identity story; this section covers the cert side.

- **`ab` is a cert-distribution target.** `ab` is in `CERT_CAPABLE_MODULES` (commit `af5b04ea`), so the LE Certificates page offers it as a `module_type` you can add as a target on a managed cert; the hub pushes the cert material to the ab agent via the generic `INSTALL_CERT` command (the agent implements it on the hub-agent side). The target badge renders with a purple **★** prefix so the ab target is visually distinct from device targets.
- **Tag AppBuilder — the pin lives in the Manage modal.** A cert is designated the AppBuilder mTLS identity by checking **★ AppBuilder identity** at the top of the Manage (targets) modal (commit `dfd2ee5f`). The checkbox POSTs `/api/le/certs/{domain}/ab` `{enabled}` → `global_config['ab_cert_identities']`, the pinned list the hub's `HUB_REQUEST` channel authorizes against (admin-only; `le_set_ab`). The toggle reflects the actual pinned state on reload (case-insensitive match on both sides, commit `ed651d72`). Tagging a cert that doesn't yet carry the clientAuth EKU auto-re-issues it with the classic profile (see below). The earlier per-row "Tag AppBuilder" button is gone; its guidance lives in Manage.
- **Persistent AppBuilder identity banner.** A banner above the certs table always shows the pinned AppBuilder cert(s) (commit `95e02058`) — purple when tagged, with a deployed-to-ab-target check and a "multiple tagged" warning; grey guidance pointing at Manage → checkbox when none. Reconciled from `c.ab` (the hub's pinned list), so you no longer have to scan every row to find which cert is the identity.
- **clientAuth EKU — a checkbox inside Manage, auto-enabled with AppBuilder.** clientAuth is toggled from a checkbox inside the Manage modal (not a top-level row button); checking it re-issues the cert now with the `classic` ACME profile (serverAuth + clientAuth) and re-distributes (commit `5e553676`). A AppBuilder cert *must* carry clientAuth to work as an mTLS client cert, so tagging AppBuilder auto-enables clientAuth when it isn't already set; the cert row shows a small `clientAuth` tag when set. Endpoint: `POST /api/le/certs/{domain}/clientauth` `{enabled}` → `LE_SET_CLIENTAUTH` on the le spoke; `LE_ISSUE_CERT`'s `client_auth` flag requests the same profile at issue time.
...
- **Split cert toolbar; dropped empty AppBuilder banner.** The cert toolbar is split into two rows (commit `cef25b2d`): actions (Issue / Renew / Distribute / mTLS status / DNS creds) on a nowrap top row, Fan-wildcard / Coverage / Retry on a second row — stops the buttons word-wrapping. The empty "no cert tagged" AppBuilder banner was removed; its guidance now lives in the Manage modal.

### mTLS trust model for the AppBuilder client cert

The hub's mTLS listener verifies presented client certs against a **combined** CA bundle — the private `LM_MTLS_CA` (the mTLS wildcard chain) **plus the system trust store** (LE/ISRG roots) — not a pinned CA only (commit `03f322a4`). Without the system store, an LE-issued, SAN-pinned AppBuilder cert failed `CERT_OPTIONAL` verification and the TLS handshake died before HTTP, stranding ab in a reconnect loop. The spoke `/ws/agent` listeners do the same via `load_default_certs(CLIENT_AUTH)`, restoring the symmetry the client leg already had (`create_default_context` trusts system + CA). This is safe because the handshake is **not** the gate here: `CERT_OPTIONAL` only makes a presented cert readable; authority is app-layer — ordinary spokes by session key, the `HUB_REQUEST` channel by AppBuilder SAN pinning (`_hub_request_authorized`). A peer presenting any publicly-trusted cert passes TLS but gains nothing unless its SAN is explicitly pinned. The combine falls back to `LM_MTLS_CA` only if the system-store read fails, so it can never brick the listener. See [lm-hub.md](lm-hub.md) › Mutual TLS.

- **Hub trust diagnostic.** `GET /api/mtls/trust-diag` (admin-only; commit `24ab0ca9`) dumps what the hub's client-verify path actually trusts and whether it would accept each pinned AppBuilder cert: the `LM_MTLS_CA` chain in full, the combined-bundle cert count, any same-subject collisions (the real-vs-private `ISRG Root X1` hazard), and an `openssl verify -purpose sslclient` of each pinned cert's live chain (pulled from the le spoke) against the exact bundle the listener uses. Backed by `mtls.trust_diagnostics()` + `mtls.verify_chain()`. Use it to see, from the WebUI, *why* a AppBuilder cert is rejected rather than guessing at the TLS error.
...
- **"AppBuilder won't connect over mTLS / 'did not receive a valid HTTP response'."** The hub's client-verify path must trust the LE-issued AppBuilder cert. Make sure the cert is tagged AppBuilder (Manage → **★ AppBuilder identity**) so its domain is in `ab_cert_identities`, that it carries the `clientAuth` EKU (the checkbox auto-re-issues with the `classic` profile), and that it's been deployed to the `ab` target. Then check `GET /api/mtls/trust-diag` — it runs `openssl verify` of each pinned cert's live chain against the hub's combined bundle and reports the exact reason for a rejection (a cert that's issued but not deployed, or a cert whose chain isn't in the system store, shows up here). See "mTLS trust model for the AppBuilder client cert" above.

## Related pages

[architecture-topology.md](architecture-topology.md), [lm-hub.md](lm-hub.md), [opnsense.md](opnsense.md) (OPNsense `OPNSENSE_INSTALL_CERT` is a cert-distribution target), [ab.md](ab.md) (AppBuilder agent/identity; this doc covers its cert side), [install-flags.md](install-flags.md).