...
> **The bridge spoke is primarily a role now.** The pxmx **bridge spoke** runs mainly as the **`proxmox`** role hosted by the agent (`agent-<hostname>`, unit `lm-agent`): the agent opens a sub-spoke `{agent}-proxmox` (module_type `hypervisor`, parent-auto-approved) and self-installs it via `agent/src/agent_spoke.py::_install_role` — cloning `lbockenstedt/pxmx.git` + deps and running `install_pxmx.sh --infra-only` for the idempotent host prep (agent-host listener). The dedicated `lm-pxmx.service` / `install_pxmx.sh` `<hostname>-spoke` path is the **legacy/standalone** alternative. (Note: this is separate from the pxmx **per-host agent** `lm-pxmx-agent.service` described below, which still runs on each Proxmox node and does the actual `qm`/`pct` work regardless of how the bridge spoke is deployed.)
...
- **`default` is the ADMIN tenant, not "All tenants"** — the WebUI tenant picker always sends `?tenant=<id>`, and `default` is the built-in **ADMIN** tenant. Selecting it must **not** accumulate every tenant's hypervisor data (the reported leak). `/api/pxmx/nodes`, `/api/pxmx/vms` and `/api/pxmx/drive-health` return a clean empty envelope flagged `select_tenant: true` (the UI then prompts *"Select a tenant"*), and the `/api/pxmx/agents` roster is scoped to UNASSIGNED + explicitly-`default` + shared agents only. The per-agent pin (`client_simulation.tenant_id`) still wins over the parent spoke's `module_metadata` binding. Only a call with **no** `?tenant=` at all (programmatic, never the WebUI) sees the whole fleet. Shared predicates: `access.tenant_scope_ids` / `access.in_tenant_scope`; same rule `routes/nw.py` names *"ADMIN(default) must not accumulate across tenants"*.
...
- **Post-clone settle reboot (+15 min)** — every clone (first-clone + reclone) stamps `post_prov_reboot[vmid]` in `usb_state.json` and the provision loop reboots the VM ~15 min later (env `POST_PROV_REBOOT_DELAY_S`, default 900) so it restarts after settling + pulling engine config + running `update.sh`. Two reboots are intentional — the immediate post-clone reboot only sets hostname/first-boot bits and the box doesn't stay up long enough to pull engine config; the +15-min one finalizes. Survives agent restart (persisted). See *How it works → Post-clone settle reboot*.
...
**Post-clone settle reboot (+15 min).** Every clone — first-clone *and* reclone, both of which end at the `set_assignment` choke point — stamps a deferred reboot into `usb_state.json`: `post_prov_reboot[vmid] = {cloned_at, reboot_at: cloned_at + 900s, bus, image_num}`. The provision loop sweeps this queue at the top of every tick (`_run_post_prov_reboot_queue`, alongside the post-prov retry queue) and fires a graceful QGA reboot once `now >= reboot_at`, then pops the entry; stale entries (bus mismatch / VM gone) are dropped without rebooting. Because it lives in `usb_state.json`, a pending reboot survives an agent restart. The delay is env-tunable: `POST_PROV_REBOOT_DELAY_S` (default 900).

There are deliberately **two** post-clone reboots, and both are intentional:
1. The **immediate** post-clone reboot (`_clone_and_provision`) applies the hostname + `sim_phy` + first-boot bits and runs `update.sh`. The guest doesn't stay up long enough to have pulled a placement/config push from the engine yet, so this reboot is *not* the "final" restart. It is **preceded by a 1-minute settle window** (`FIRST_REBOOT_SETTLE_S`, default 60s) measured from when the guest's QGA agent first answers — the freshly-cloned box is left running for at least that long before the reboot fires, so filesystem/services can settle. (Without it the reboot fired the instant the hostname/sim_phy scripts finished, which corrupted some clients.)
2. The **+15-min settle reboot** (the queue above) restarts the box *after* it has settled, pulled its engine config, and run `update.sh` — so it comes back fully configured. (Reclone does *no* immediate reboot, so for reclone this is the first/only post-clone restart — still correct: the settle+config window is what matters, not which reboot is "first.")

The pending reboot is visible in the WebUI: each freshly-cloned VM shows a blue **🔄 Reboots in Xm** badge next to its status (live countdown) with a tooltip explaining the two-reboot rationale. The schedule rides the per-VM `usb_state[]` telemetry (`reboot_at`/`cloned_at`) up through the existing cs spoke relay.
