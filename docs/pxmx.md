...
1. The **immediate** post-clone reboot (`_clone_and_provision`) applies the hostname + `sim_phy` + first-boot bits and runs `update.sh`. The guest doesn't stay up long enough to have pulled a placement/config push from the engine yet, so this reboot is *not* the "final" restart.
