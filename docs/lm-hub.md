...
- **"The feed is running but no spokes appeared."** If they are missing from Spokes & Agents, the synthetic spokes connected but were not approved — check the feed's output panel for the child's own errors. If they are in Spokes & Agents but the **Simulations** views are empty, the feed is bound to the shared tenant: name a real tenant in the Tenant field and restart the feed.
