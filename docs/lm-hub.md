...
- **"The feed is running but no spokes appeared."** The synthetic spokes connected but were not approved, so their telemetry is discarded. Check the hub has a shared tenant (Start refuses without one) and look at the feed's output panel for the child's own errors.
