...
> **Primarily a role now.** LDAP runs mainly as the **`ldap`** role hosted by the agent (`agent-<hostname>`, unit `lm-agent`): the agent opens a sub-spoke `{agent}-ldap` (module_type `directory`, parent-auto-approved) and self-installs it via `agent/src/agent_spoke.py::_install_role` (clones `lbockenstedt/ldap.git` + deps). The dedicated `lm-ldap.service` / `install_ldap.sh` `ldap-spoke-1` path is the **legacy/standalone** alternative. Connection config (`LDAP_SERVER_URL`/admin DN/PW/base DN) comes from the hub push (WebUI), not a per-module `.env`. A Global Admin sets the **server connection** in **Setup → Directory (LDAP)** (`GET`/`POST /setup/ldap-config`, `core/src/routes/ldap.py`), stored under `global_config["ldap"]` (`base_dn`/`admin_dn`/`admin_pw`/`server_url`/`server_id`/`mirror_peers`; the admin password lives only inside the Fernet-encrypted state and is never echoed on GET — masked to `admin_pw_set`). Save persists and re-pushes `UPDATE_CONFIG` to every connected directory spoke via `hub.push_ldap_config_all()`. These values take **precedence** over the legacy `ldap_instances` entry and the `dc=example,dc=org` install default — `LabManagerHub._ldap_config_for_spoke` merges them per field (`routes.ldap.merge_ldap_connection`). This is the SERVER config, not per-tenant (TENANT == OU is unchanged).
...
- **Hub connection.** Like every LM spoke, ldap dials the hub over a WebSocket. In the current unified model this is the sub-spoke `{agent}-ldap` opened by the agent (parent-auto-approved); the legacy path is the standalone `ldap-spoke-1` dialing the hub directly. Either way the wire protocol is identical.
