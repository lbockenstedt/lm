...
- **Scheme selection:** same-box → `ws://127.0.0.1:<port>`; remote + mDNS TXT `tls_port` → `wss://<ip>:<tls_port>`; remote no TXT → `ws://<ip>:<port>` (legacy cert-less hub). `agent_listener=True` reads TXT `agent_port` instead of the spoke-WS port.
- **Same-box = IP-equality, NOT mDNS receipt.** mDNS crosses the LAN (L2-scoped), so hearing the hub over mDNS does not mean same-box. `is_hub_local(hub_ip)` compares the resolved/mDNS hub IP against this box's own interface IPv4s (UDP-connect-to-`223.255.255.1` + psutil, loopback included). The bind addresses (127.0.0.1-only plain) enforce it.
