...
LM is a zero-trust hub/spoke/agent management mesh for a lab/DC lab. One **hub** (the `lm` repo) is the control plane + WebUI + state store. It talks to many **spokes**, each wrapping one external system (a Proxmox cluster, an OPNsense firewall, NetBox IPAM, ClearPass NAC, an LDAP directory, Kea DHCP, Unbound DNS, a fleet of switches, a certbot ACME producer, a client-simulation engine). A few spokes **bridge** further out to **agents** that run on remote hosts (pxmx per-host agents on Proxmox nodes, the agent-spoke that hosts module roles, bugfixer as an agent-type client).
...
```
   ...
   agent-spoke  (wss to hub /ws/spoke; hosts module roles as sub-spokes)
...
- **Agents** — three flavors:
  ...
  - **agent-spoke** (`lm/agent/src/agent_spoke.py::GenericAgent`) dials the hub `/ws/spoke`. It is the "call home, then morph into a role later" shape — one agent per node hosting module roles as sub-spokes (see the unified-agent deep dive below). (The legacy `generic_agent/` leaf was removed — commit `3ddda2c`.)
...
- **Verify-OFF by default** — `ssl._create_unverified_context()` (`CERT_NONE`): traffic is encrypted but the self-signed hub cert is **not** authenticated (on-path MITM-able; lab-acceptable). This is the default for every spoke/agent client (`BaseControlPlane._client_ssl_ctx`, mirrored in the pxmx agent and the agent-spoke).
...
The agent-spoke role loader (`lm/agent/src/agent_spoke.py::_ROLE_MAP`) maps a `--role` to the spoke class + repo URL so a single agent box can morph into any of these on `LOAD_ROLE`.
...
**After the clone — the +15-min settle reboot.** Both clone paths (first-clone `_clone_and_provision` and reclone `_reclone_vm_core`) end at `set_assignment`, which stamps `post_prov_reboot[vmid] = {cloned_at, reboot_at: cloned_at + 900s, bus, image_num}` into `usb_state.json`. The provision loop sweeps this queue at the top of every tick (`_run_post_prov_reboot_queue`, unconditionally — even with auto-prov off) and fires a graceful QGA reboot once `now >= reboot_at` (with a `qm reset` fallback), then pops the entry; stale entries (bus mismatch / VM gone) drop without rebooting. Because it's persisted, a pending reboot survives an agent restart. Delay is env-tunable: `POST_PROV_REBOOT_DELAY_S` (default 900). Two reboots are intentional — the immediate post-clone reboot only applies hostname/first-boot bits (the box doesn't stay up long enough to pull engine config); the +15-min reboot restarts it *after* it has settled, pulled engine config, and run `update.sh` so it comes back fully configured. The immediate reboot is itself preceded by a **1-minute settle window** (`FIRST_REBOOT_SETTLE_S`, default 60s) measured from when the guest's QGA agent first answers — the freshly-cloned box is left running at least that long before the reboot fires so the filesystem/services can settle (without it, rebooting the instant the hostname/sim_phy scripts finished corrupted some clients). The pending reboot shows in the WebUI as a blue **🔄 Reboots in Xm** badge (live countdown + tooltip) next to the VM's status, riding the per-VM `reboot_at`/`cloned_at` telemetry fields up the existing relay.
