...
| A13 | `lm/core/src/main.py:2433,2479` `collect_all_logs`/`collect_error_logs` | `os.listdir` + per-file `open` + `deque` then inline `json.dumps` binary search; in-code comment notes this "stalled the event loop on every AppBuilder poll" | `await asyncio.to_thread(collect_all_logs, ...)` from `handle_hub_request` + `/setup/logs*` handlers | S |
...
| A16 | `lm/core/src/hub_bug_store.py:30-85,90-121` `_store_bug_report`/`_get_bug_report` | POST `/api/bug-report` + GET_BUG_REPORT (ab): makedirs + 3 writes + base64 decode + screenshot | `asyncio.to_thread` both | S |
