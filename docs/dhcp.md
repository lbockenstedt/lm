...
**Configure it.** Normal setup is automatic. When exactly two connected agents in the DHCP tenant report an active `dhcp-server` role, `POST /api/dhcp/ha/discover` enrolls them as primary/standby, selects their private local addresses, repairs their `dhcp_instances` inventory records, generates the worker PSK and HA control credential, and issues a unique mutual-TLS certificate to each node from a persistent coordinator-owned CA. The WebUI runs this discovery for Global Admins when DHCP is opened. One discovered server waits for its partner; more than two fails closed rather than choosing an arbitrary pair. Public/NAT WebSocket source addresses are never used.

The Global-Admin-only DHCP → **Diagnostics** topology editor and `POST /api/dhcp/ha` remain available for exceptional manual layouts. Manually supplied worker secrets and HA credentials are write-only: omit them on a re-save and the stored values are carried forward. A rolled-back change also restores the previous worker PSK, so already-provisioned Kea workers keep authenticating.
...
- **IMPORTANT — two separate Kea instances.** This module's Kea is the site's real production Kea DHCP4 server, reached via its Control Agent at `KEA_URL` (current code default `http://localhost:8001`). This is **completely distinct** from the `cs` (Simulations) role's own Kea instance, `kea-dhcp4-sim`, whose Control Agent listens on `127.0.0.1:8102` and which only serves the simulated-client network `169.253.1.0/24` for auto-provisioning test VMs. The simulation Kea is never involved in real subnet/lease/reservation management. Don't point one at the other's port. **Note:** the sim ctrl-agent previously defaulted to `:8002` — the *same* port this module's own HA peer channel (`kea-ha-agent`, see below) binds to — which silently broke DHCP HA on any host running both roles (the HA hook lost the port race and failed to load, leaving the HA pair stuck `down`/`partner-down`). The sim's port was moved to `:8102` in `cs/lm-spoke/install_cs.sh` to eliminate the collision; a host still showing `kea-ctrl-agent-sim` bound to `:8002` predates that fix and should be reinstalled/updated.
...
- **"Is this the same Kea used by the client/USB simulation feature (`cs` module)?"** No. This `dhcp` module manages the site's real Kea DHCP4 server via its Control Agent (`KEA_URL`, code default `:8001`). The `cs` (Simulations) role runs its own separate Kea instance, `kea-dhcp4-sim`, with its Control Agent on `127.0.0.1:8102`, serving only the simulated-client network `169.253.1.0/24` for auto-provisioning test VMs. They're independent Kea processes/configs, even on the same host — never point one module's `KEA_URL` at the other's port.
- **"DHCP HA never converges / the HA pair is stuck `down` or `partner-down`, and every apply fails with `hook libraries failed to load`."** Check whether something else on either host is already bound to the HA peer port (`--ha-port`, default `8002`) — `ss -tlnp | grep 8002`. A known cause: an older `cs`-role install with `kea-ctrl-agent-sim` still on its pre-fix default of `:8002` (now `:8102`, see above) wins the bind race against this module's `kea-ha-agent`, which then crash-loops with `Address already in use` and never joins the pair. Reinstall/update the `cs` role to move it off `:8002`, then restart `kea-ha-agent` and re-sync.
