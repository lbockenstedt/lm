...
AppBuilder is two things wearing one hat: (1) an autonomous bot that watches your GitHub
...
there, the hub relays it to AppBuilder's LLM layer and streams back an answer grounded in
the docs. It has its own dashboard at `http://<ab-host>:8000` (issue queue, config,
...
- **Hub heartbeat at INFO — heartbeat-triage gate is INVARIANT.** The hub's `run_hub_heartbeat_loop` emits a greppable `[heartbeat] ok module=hub …` line every ~60s (`LM_HEARTBEAT_INTERVAL_S`, default 60) at **INFO, not DEBUG** — at the default INFO level a DEBUG line is filtered before it reaches `HubLogHandler`, never buffered into `self.logs`, never returned by `GET_LOGS`, and AppBuilder never observes the hub's own heartbeat. AppBuilder's `scan_heartbeats` is gated on TWO conditions, both of which must hold before any per-spoke triage runs: (1) the agent is approved and a signed `GET_SPOKE_STATUS` actually round-trips to the hub, and (2) the hub's own `[heartbeat]` line has been observed at least once since this AppBuilder process started. Suppressing until both hold is what prevents a false "hub unreachable — every spoke missing" reinstall flood after a reinstall or boot. A `HEARTBEAT_WARMUP_S` (300s) backstop since (re)approval lets a genuinely-broken hub-heartbeat loop be triaged — but hub-only, never the spokes, so a dead pipeline still can't flood. `heartbeat_triage_enabled` (default OFF) is the top-level Settings toggle for the whole triage path.
- **AppBuilder mTLS cert + LE tag flow (cert trust) — INVARIANT.** AppBuilder's identity is an LE-issued cert: **one cert serves both its WebUI/server leg and its mTLS client leg**. The hub's reverse `HUB_REQUEST` channel is **cert-bound, not `spoke_id`-bound** — `_hub_request_authorized` (`core/src/main.py`) authorizes a request only when the calling connection presented a client cert whose SAN matches the pinned `global_config['ab_cert_identities']` list; `spoke_id` is hostname-derived and spoofable (name a box `ab`), so it is NOT the gate. Fail-closed: no pinned cert, no client cert presented, or SAN mismatch → denied and logged. `ab` is in `CERT_CAPABLE_MODULES` (`core/src/cert_distribution.py`), so the LE module can target it; the LE Certificates page has a "★ AppBuilder identity" checkbox inside the cert's **Manage** modal (`POST /api/le/certs/{domain}/ab`) that adds/removes the domain in the pinned list, and stars `ab` targets so the cert is deployed to the ab agent. A persistent purple "★ AppBuilder identity" banner above the cert table shows which cert is pinned (and whether it's deployed to a `ab` target — flagging extras). The hub's client-cert verify path trusts the **system store** in addition to `LM_MTLS_CA` (`server_client_ca_file()` in `core/src/security/mtls.py` concatenates the Hub Local CA + retired CA + legacy `LM_MTLS_CA` + the system root bundle via `certifi`/OpenSSL paths) so an LE-issued, SAN-pinned AppBuilder cert verifies without having to live in the private mTLS CA — mirroring the client leg's `create_default_context` symmetry. The admin-only `GET /api/mtls/trust-diag` surfaces what the hub trusts, per-connection mTLS state (who is actually presenting a verified client cert vs. cert-less permissive fallback), and a live check of each pinned AppBuilder SAN against connected clients. See [le.md](le.md) for the one-cert-per-target cert-target model.
...
- **Issue-fixing loop** (`poller_worker` → `run_scan_cycle` → `scan_repo_issues` in
  ...
  `POLL_INTERVAL_SECONDS`, tighter during configured work hours) AppBuilder lists open
  ...
  scanning), skipping issues already resolved/failed or marked `ab-dismissed`.
...
- **Deployment decision.** A fix only gets pushed directly to the default branch when
  the repo is in `trusted_repos`, `direct_push_enabled` is on, AppBuilder owns the repo
  (or it's the self-diagnosis repo), and the reviewer approved. Otherwise AppBuilder opens
...
- **"Ask AI" / help assistant.** The hub owns the docs corpus and the tool-calling loop
  ...
  by keyword match, builds a system prompt, and relays each model turn to AppBuilder over
  the hub connection as an `HELP_ASK` command (`hub_agent.py::_handle_message`).
  AppBuilder runs exactly one `call_llm` turn using whatever LLM provider is configured
  ...
  `true` only when a AppBuilder agent connection is present in the hub's
...
- **"File a Bug" / feature-request pipeline.** The LM footer's "🐞 Bug/Feature
  ...
  marker line (`core/src/hub_bug_store.py`). AppBuilder scans the raw hub logs
  ...
  **Feature requests are gated on admin approval** — the hub annotates each
  ...
  the AppBuilder Diagnostics "LM Feature Request Ingestion" card can count them
  ...
  does the report become visible to AppBuilder. Status flows `pending` →
  `approved` → `filed` (issue opened) → `fixed` (AppBuilder closed the issue). A
...
- **Runtime-error banner auto-files a bug.** The WebUI's error boundary
  ...
  agent; AppBuilder picks it up through the same `[bug-report]` pipeline. Dedup is
...
- **Connecting to the hub.** AppBuilder is a hub **agent**, never a spoke: it authenticates
  ...
  scheme itself (`MessageSigner`) so AppBuilder never has to import the `lm` core package —
  ...
  `SET_LOG_LEVEL` broadcasts (the WebUI "Enable Debug" button flips AppBuilder's log level
  too).
- **Watchdog + rollback.** `ab-watchdog.service` (`watchdog.py`) polls
  `/api/health` every 5 seconds. When an auto-update is pending (`update_pending` file
  in `/etc/ab/`), it makes sure the running process is actually on the new commit
  ...
  --hard`s `/opt/ab` back to the last known-good commit and restarts the service —
  so a broken self-update can't strand AppBuilder down.

## How to use it

1. **Point AppBuilder at repos and labels.** Edit `/etc/ab/config.json` (or the
...
2. **Configure at least one LLM provider.** From the dashboard's LLM setup page, add
   ...
   Studio, or the `claude_cli` slot which needs no API key). AppBuilder won't attempt a
...
3. **Connect it to the hub** (enables both the hub-relayed log/update tooling and the
   ...
   `HUB_AGENT_ID` (defaults to `ab`), and optionally a pre-shared `HUB_AGENT_SECRET`
   / `HUB_SECRET` in `config.json`, then restart the `ab` service. With no secret it
   connects zero-touch and waits for approval in the hub's **Setup → Spoke Approvals**
   (AppBuilder shows up there like any other agent, even though it isn't a spoke).
4. **Verify "Ask AI" is live.** Once AppBuilder is connected and approved, the WebUI's
   help button should appear; `GET /api/help/available` on the hub should return
   `{"available": true}`.
5. **Pin a AppBuilder mTLS cert (enables the reverse `HUB_REQUEST` channel).** Until a
   cert is pinned the channel is fail-closed — AppBuilder connects and can be relayed
   `HELP_ASK` answers, but cannot pull hub logs or file bugs. From the LE Certificates
   page, issue a cert for a AppBuilder hostname (or pick an existing one), open **Manage**,
   tick **★ AppBuilder identity** (`POST /api/le/certs/{domain}/ab`), and add a
   `ab` target so the cert is deployed to the agent. The persistent purple
   "★ AppBuilder identity" banner confirms the pin. Verify with
...
- **The "Ask AI" button is missing in the WebUI.** This means the hub's
  `/api/help/available` check found no connected AppBuilder agent. Check that the
  `ab` service is running, that `HUB_WS_URL` is set and reachable, and that the
...
- **Issues aren't getting fixed even though they're labeled correctly.** Check, in
  ...
  already carry `ab-dismissed` (that label permanently skips it until removed)?
- **AppBuilder is offline / the hub can't reach it.** AppBuilder connects to the hub *as a
  client* — it's not a listener the hub dials into — so "offline" almost always means
  the `ab` systemd service isn't running, or `HUB_WS_URL` is wrong/unreachable
  from the AppBuilder host. Its own dashboard on port 8000 is independent of the hub
  connection, so the dashboard can be up while the hub connection is down; check
  `hub_agent_status` on the AppBuilder dashboard to see the connection state directly.
- **A self-update seems to have broken AppBuilder.** The watchdog (`ab-watchdog`
  service) should catch this automatically within about a minute and roll back to the
  last known-good commit — check `/var/log/ab_watchdog.log` for a rollback entry.
