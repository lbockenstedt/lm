...
| `--id <id>` / `--name <id>` | Spoke ID. Unified default is the **agent** id `agent-<hostname>` (one agent hosting all module roles); the per-module `<hostname>-spoke` / `<hostname>-spoke-1` default is the **legacy/standalone** path |
...
`--reinstall` (reinstall over existing), `--reset-secrets` (wipe spoke/hub secrets), `--reset-users` (wipe user accounts), `--exclude <csv>` (skip module roles — `cs,pxmx,opnsense,cppm,netbox,ldap,dns,dhcp,nw,le`), `--hub-only` (install **just the hub** — no co-located module roles; equivalent to excluding every module), `--tls-verify` (optional `--tls-ca-cert <path>`; defaults CA to the hub's own `$TLS_CERT`), `--no-setup-token` (leave the first-run `/auth/setup` open — dev/loopback only).

**Unified agent-spoke model:** installs ONE agent (`agent-<hostname>`) that hosts every module as a **role** — it maps each module → its `_ROLE_MAP` role and runs a single `install_agent.sh --id agent-<hostname> --roles <csv> --loopback`, instead of ten dedicated `*-spoke-1` installs. `--loopback` marks the agent co-located with the hub (hub owns `:443`): the pxmx role's agent-host listener binds `127.0.0.1:8443` (hub `/ws/agent` byte-proxies) and the cs role's `:443` listener is suppressed. Only the **agent id** is pre-approved; each role sub-spoke `{agent}-{role}` auto-approves via the parent. Connection config (NetBox URL/token, OPNsense host/key, …) now comes from the **hub push** (configure it in the WebUI), not a per-module `.env`; the NetBox *application* remains a separate install.

### `install_menu.sh` — interactive bootstrap (single master installer)
One entry point that drives every other installer. Top menu:
- **`1) Hub`** — spoke checklist → `install_all.sh --exclude <unselected>`.
- **`2) Agent`** — role-capable leaf → `agent/install_agent.sh`. Prompts `--hub` (default `auto`), `--id`, first secret, hub root secret, "Verify hub TLS certificate?", clone-only, then a **module-role checklist** (defaults to none = bare agent that loads roles later; toggle to pre-load) mapped to `--roles <csv>` via the same `MODULE_ROLE` map as `install_all.sh` (`cs→simulation, pxmx→proxmox, opnsense, cppm, netbox, ldap, dns, dhcp, henet, nw→network, le`). Selecting `cs`/simulation makes this box host the client-sim `/ws/agent` listener (`install_agent.sh` sets `LM_CS_AGENT_LISTENER=1` for a non-colocated simulation role).
- **`3) Proxmox Host Agent`** — installs/uninstalls the pxmx **node-agent** (separate `pxmx` repo, driven via curl of `pxmx/agent/install_agent.sh`); prompts the spoke IP (`--spoke-ip`, blank → auto-discover). This node-agent reports to a spoke hosting a `/ws/agent` listener (a pxmx-role spoke, or a simulation-role box with its listener enabled).
- **`4) Uninstall`** → `uninstall.sh` (guarded master teardown).

Env `LM_BRANCH` (default `main`). Re-exes as root via sudo; re-execs from a temp file with stdin on `/dev/tty` for the `curl | bash` one-liner (the legacy `generic_agent` leaf was removed — the Agent path uses the BaseSpoke-based agent).

### `agent/install_agent.sh` — the unified installer (every node is an agent)
`--hub` (optional; omit/`auto` → auto-discover), `--id` (default `agent-<hostname>`), `--secret`, `--hub-secret`, `--role <one>` / `--roles <csv>` (of `dns|dhcp|network|netbox|opnsense|ldap|simulation|cppm|proxmox|le|console|statuspage|proxy|truenas`), `--loopback` (agent is **co-located with the hub** — pxmx role binds `127.0.0.1:8443`, cs role `:443` listener suppressed; standalone omits it → listeners on `:443`), `--clone` (clone-only; id derives from hostname at runtime), `--tls-verify`/`--tls-ca-cert`.
...
### Other lm installers
`install_production.sh` (one-liner wrapper — curls `install_all.sh` from `main` and execs it; args pass through, so `install_production.sh --hub-only` works), `prep_for_imaging.sh`, `start_all.sh`, `sync_secrets.sh`, `verify_auth.sh`. The hub installs via `install_all.sh` (or `install_all.sh --hub-only` for the hub alone); every module onboards as a remote spoke/role via `agent/install_agent.sh` (see § pxmx/cs/… below for the module-repo installers the role loader invokes for host prep); the WebUI ships in the checkout at `/opt/lm/WebUI` and is served directly by the hub; and the systemd `lm.service` unit runs the hub.
