...
Top menu: `1) Hub` (spoke checklist → `install_all.sh --exclude <unselected>`) or `2) Agent` (→ `agent/install_agent.sh` — the BaseSpoke-based GenericAgent; the legacy `generic_agent` leaf was removed). Hub path prompts co-located spokes; Agent path prompts `--hub` (default `auto`), `--id`, first secret, hub root secret, "Verify hub TLS certificate? (requires the hub CA cert) [y/N]", clone-only. Env `LM_BRANCH` (default `main`). Re-exes as root via sudo; re-exec from a temp file with stdin on `/dev/tty` for the one-liner.
