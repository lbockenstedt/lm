...
**The worker is not a generic agent.** Its op table is fixed at import: `DNSW_APPLY`, `DNSW_STATE`, `DNSW_STATUS`, `DNSW_DIAGNOSTICS`, `DNSW_STATS`, `DNSW_FORWARDERS`, `DNSW_FORWARDER_ADD`, `DNSW_FORWARDER_UPDATE`, `DNSW_FORWARDER_REMOVE`, `DNSW_STANDDOWN`. There is no `RUN_COMMAND`, no `WRITE_FILE`, no caller-supplied path or URL. The conf path is fixed at start from local config. Forwarders are per-resolver, so the **Forwarders** tab in cluster mode aggregates `DNSW_FORWARDERS` from every member (tagged with the member it came from) instead of reading the coordinator box's own Unbound, which may not exist. Adds fan out to every member and roll back successful members if any resolver rejects the change.
...
**Configure it.** Active DNS Server roles installed through LM are discovered and enrolled automatically when an administrator opens the DNS module. The generic agent reports its private local service address; the hub never substitutes the public/NAT source address of the agent's WebSocket. DNS Management mints or reuses the worker credential, hands the public coordinator certificate and ephemeral credential to the server agent over the authenticated control channel, and the agent idempotently installs `lm-dns-worker`. After the worker connects, DNS Management adopts its existing records before allowing any mutation; an explicitly unloaded/disabled DNS Server is never re-enabled by discovery. The discovered server is also added to Setup inventory. Worker ID and Worker Secret are only needed for an external/manual resolver that was not installed as an LM role. DNS → **Diagnostics** → *Configure resolver cluster* remains the advanced direct topology editor.
...
…plus `--ca-cert <coordinator cert>`, which is **required** — the worker verifies the coordinator before sending its secret. That installs Unbound **and** the `lm-dns-worker` unit; it implies `--infra-only` (the module lives on the coordinator, not here). Both the hosted management role and standalone coordinator install create `/etc/lm-dns/tls` and mint a self-signed coordinator certificate when one is absent (standalone installs may override it with `--tls-cert`/`--tls-key`/`--tls-san`). Copy `/etc/lm-dns/tls/coordinator.crt` to each resolver and pass it as `--ca-cert`. One configured worker enables remote management; two or more add redundancy.

**See it.** DNS → **Diagnostics** grows a *Resolver cluster* panel (per-member convergence, applied version + digest, Unbound up/down, last-seen, and the last commit's per-member errors) plus each member's own diagnostics findings. `GET /api/dns/cluster` returns the same report; Settings → Diagnostics carries a one-line summary. A non-admin sees the verdict but not member hostnames, digests or error text. Each member's row shows a human-friendly **display_name** (e.g. `MIPBE-SVCS1`) as the primary label instead of the raw member id — sourced from the DNS managed-device inventory (`dns_instances`, itself named from `module_names`/`module_metadata` at discovery time) or, failing that, from `module_names`/`module_metadata` directly; a member with no known name falls back to showing its raw id as before. The raw id (the UUID/agent-id actually used for cluster membership) is never dropped — it stays underneath the name and in a hover tooltip, so identity stays traceable. `display_name` is additive on both `GET /api/dns/cluster` and the diagnostics `cluster` block — existing consumers that only read `id` are unaffected.
...
- **Command flow.** WebUI/hub issues one command at a time over the hub↔spoke session: `GET_VERSION`, `UPDATE_CONFIG`, `DNS_STATUS`, `DNS_LIST`, `DNS_ADD`, `DNS_DELETE`, `DNS_UPDATE`, `DNS_SYNC`, `DNS_STATS`, `DNS_FORWARDERS`, `DNS_FORWARDER_ADD`, `DNS_FORWARDER_UPDATE`, `DNS_FORWARDER_REMOVE`.
...
- **Per-destination-name query stats.** `unbound-control stats_noreset` only aggregates by type/rcode/etc. — it has no counter per queried *name*. To surface "A record for www.dwx.com — 42 queries", `UnboundManager` self-enables Unbound query logging on first `get_stats()` call by writing a managed conf.d snippet (`/etc/unbound/conf.d/lm-logging.conf`: `log-queries: yes` + `logfile: /var/log/unbound/lm-queries.log`) and reloading, then incrementally tails that log (tracking a byte offset, so repeated polls only parse newly-appended lines; handles rotation via inode/size checks). Counts are kept in memory per `name|TYPE`, capped at `MAX_TRACKED_NAMES` (5000) distinct entries to bound memory — once hit, already-tracked names keep incrementing but brand-new names are dropped until a service restart clears the table. `get_stats(search=...)` (relayed via `GET /api/dns/stats?search=`) filters the per-name breakdown by case-insensitive substring match on the name, and the API response is further truncated to the top `TOP_NAMES_LIMIT` (200) by count. Each log line also carries the querying CLIENT IP (Unbound's `log_query_in` format, `<ip> <qname> <type> IN`), so every tracked entry is actually keyed `name|TYPE|source_ip` internally and exposed as `query_names: [{name, type, count, sources: [{ip, count}, ...]}, ...]` (sorted by count desc, sources sorted by count desc) and `query_names_tracked` (total distinct name/type/source entries currently tracked) alongside the existing `query_types` field. `get_stats(search=..., source_prefixes=...)` / `GET /api/dns/stats?search=` also accepts `source_prefixes` (a list of CIDRs) to scope BOTH which rows are returned and their counts to only sources inside those prefixes — this is how a tenant's DNS statistics view is restricted to only their own devices' queries (an empty-but-non-None list fails CLOSED: no rows, not the unfiltered fleet). The hub route derives `source_prefixes` the same way `_filter_tenant` does for DNS records: the effective tenant's NetBox prefixes when one is selected (picker or session), `None` (unfiltered) for an admin with none selected. The WebUI's Overview tab renders this as a "Queries by Destination" list (each row showing its top source IPs) with a live search box. In cluster mode, `_cluster_stats` fans `DNSW_STATS` (carrying `search` and `source_prefixes`) out to every member and merges `query_names` by `(name, type)` (and their `sources` by IP) summed across resolvers, same as the existing `query_types` merge.
- **Forwarders in detail.** `DNS_FORWARDERS` parses `unbound-control list_forwards` lines (e.g. `. IN forward 8.8.8.8 8.8.4.4`) into a per-zone list of upstream servers — relayed via `GET /api/dns/forwarders`. Global Admins can manage persistent forwarders stored in `/etc/unbound/conf.d/lm-forwarders.conf`:
  - **Add:** `POST /api/dns/forwarders` (`DNS_FORWARDER_ADD`) adds a forwarding zone or merges additional upstreams into an existing zone (up to 8 addresses). A partial cluster write is rolled back.
  - **Edit/Update:** `PUT /api/dns/forwarders` (`DNS_FORWARDER_UPDATE`) updates upstream servers in-place without merging, and supports zone renaming when `old_zone` is provided.
  - **Delete:** `DELETE /api/dns/forwarders` (`DNS_FORWARDER_REMOVE`) completely removes the zone's forwarding block.
  Every worker writes the configuration snippet and reloads Unbound. In cluster mode, updates and removals fan out to all members (`DNSW_FORWARDER_UPDATE`, `DNSW_FORWARDER_REMOVE`) and fail if any resolver rejects the change.
- **Forwarders in cluster mode never hard-fail on a dropped member.** `GET /api/dns/forwarders`'s `_cluster_forwarders` fanout always returns top-level `status: SUCCESS` with whatever it *could* collect, bucketing any resolver that didn't answer into `member_errors` (`{member_id: reason}`) instead of silently shrinking the list. The **Forwarders** tab renders that as an amber "N resolver(s) did not report" banner above the table rather than leaving the operator staring at a table that's missing a forwarder they just added with no error anywhere.
- **The Forwarders/Diagnostics/Overview tabs share one un-awaited DNS-worker-discovery fetch** (deduped on `window._dnsWorkerDiscovery`, gated admin-only) that re-renders the DNS view once it resolves. Because a steady-state cluster reports its already-configured workers on every call, that re-render used to fire on almost every load and replay whichever tab it was *originally* kicked off for — if the operator switched tabs in the meantime (e.g. added a forwarder on **Forwarders** right after loading **Records**), the stale discovery callback would silently overwrite the correctly-rendered tab with the other one a moment later. It now re-renders the tab that is actually on screen (`currentView`/`currentSubView`) and no-ops once the operator has left the DNS view entirely.
...
- **Check resolver health:** **Overview** tab — total queries, cache-hit ratio, recursion latency, uptime tiles, plus per-type breakdown. Useful for spotting cache-hit-ratio drops or clients hammering with retries.
...
- **Add a forwarder:** Forwarders tab → **+ Add Forwarder** → specify zone and upstream server IP addresses (`POST /api/dns/forwarders`).
- **Edit a forwarder:** Forwarders tab → Edit action on a forwarder row → update upstream addresses (`PUT /api/dns/forwarders`).
- **Delete a forwarder:** Forwarders tab → Delete action on a forwarder row → confirms and deletes forwarder (`DELETE /api/dns/forwarders`).
