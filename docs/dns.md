...
**Configure it.** Add each resolver under Setup → Module Management → DNS. Saving the server list pushes the complete topology to its associated DNS Management spoke; the Worker ID must match the worker installer's `--member-id`. The worker secret is write-only, required when the first server is added, and omitted from persistent hub state. DNS → **Diagnostics** → *Configure resolver cluster* remains the direct topology editor. Both paths send `DNS_CLUSTER_CONFIG` with `{"members": [{"id","host"}, …], "worker_secret": "…"}`; re-saving with the secret blank keeps the spoke's stored secret.
