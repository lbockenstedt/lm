...
proxy had one cookie arrive from 198.18.10.124 and 198.18.10.96 within the
120s hijack window. (The addresses here are redacted to the reserved
benchmarking range 198.18.0.0/15 — the real pair is a live corporate egress
pool and this repository is public. Only the prefix changed, so every
subnet/adjacency relation these tests assert is preserved.)
The proxy pool hands consecutive requests to different
...
@pytest.mark.parametrize("a,b", [
    ("198.18.10.124", "198.18.10.96"),   # the real incident pair
    ("10.1.2.3", "10.1.2.250"),
...
@pytest.mark.parametrize("a,b", [
    ("198.18.10.124", "198.18.11.96"),   # adjacent /24 → still a hijack
    ("1.1.1.1", "9.9.9.9"),
...
def test_prefix_is_configurable(monkeypatch):
    ...
    assert access.same_bind_subnet("198.18.10.124", "198.18.11.96") is True
    # Full width restores strict per-address binding (the old behaviour).
    monkeypatch.setattr(access, "_IP_BIND_PREFIX4", 32)
    assert access.same_bind_subnet("198.18.10.124", "198.18.10.96") is False
    assert access.same_bind_subnet("198.18.10.124", "198.18.10.124") is True
...
def test_session_survives_same_subnet_egress_change():
    ...
    sessions = {"tok": _mk_session("198.18.10.124")}
    sess = access.session_user(sessions, _FakeRequest("tok", "198.18.10.96"))
    ...
    assert sessions["tok"]["client_ip"] == "198.18.10.96"
...
def test_rebind_then_reject_from_original_far_ip():
    ...
    assert access.session_user(sessions, _FakeRequest("tok", "198.18.10.7")) is not None
...
def test_incident_regression_proxy_pool_does_not_lock_admin_out(tmp_path):
    """198.18.10.124 + 198.18.10.96 are one operator behind an egress proxy.
    ...
    tm = _mk_tm(tmp_path)
    involved = sorted(
        o for o in {"198.18.10.124", "198.18.10.96"}
        if not access.same_bind_subnet(o, "198.18.10.124"))
    assert involved == [], "owner's own egress pool must never be blocked"
    for ip in ("198.18.10.124", "198.18.10.96"):
        assert ip not in tm._blocks
    access.set_client_ip_resolver(None)
...
def test_ipv4_mapped_addresses_do_not_all_share_a_64():
    ...
    assert access.same_bind_subnet("::ffff:198.18.10.124", "::ffff:45.33.32.156") is False
    assert access.same_bind_subnet("::ffff:198.18.10.124", "::ffff:198.18.11.5") is False
    # ...while a genuine same-/24 pair still matches through the mapped form.
    assert access.same_bind_subnet("::ffff:198.18.10.124", "::ffff:198.18.10.96") is True


def test_mapped_and_native_ipv4_are_the_same_client():
    assert access.same_bind_subnet("::ffff:198.18.10.124", "198.18.10.96") is True
    assert access.same_bind_subnet("198.18.10.124", "::ffff:45.33.32.156") is False


def test_mapped_ipv4_session_is_rejected_across_subnets():
    access.set_client_ip_resolver(_peer_resolver)
    sessions = {"tok": _mk_session("::ffff:198.18.10.124")}
