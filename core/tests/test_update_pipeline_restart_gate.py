...
* ``check_update_health`` stale sentinel is NON-force (gated + 1h backstop).
* ``perform_update``'s watchdog sentinel is ``force=bool(force)`` (no
  ...
  auto-update. The 1h hard backstop (``LM_WATCHDOG_STALE_BACKOFF_S``) on the
  watchdog side guarantees a stale/buggy build still reloads within 1h even
  behind a logged-in user.
...
def test_non_force_sentinel_has_no_force_prefix():
    ...
    — the watchdog gates it on RESTART_ALLOWED + the 1h backstop."""
    with tempfile.TemporaryDirectory() as td:
        path = os.path.join(td, "stale-sentinel")
        os.environ["LM_STALE_RESTART_SENTINEL"] = path
        try:
...
def test_check_update_health_stale_sentinel_is_non_force():
    ...
    the watchdog gates it (idle / 2am) + 1h backstop. force=True here is the
    mid-day-boot regression — every autobump made the watchdog restart a logged-
    in operator immediately. The call must read force=False (or force=bool(...))
    but NOT force=True."""
    ...
    assert needle in _SRC_TEXT, (
        "check_update_health stale sentinel must be force=False (gated + 1h "
        "backstop), not force=True — force=True boots logged-in users mid-day")
...
def test_perform_update_sentinel_force_only_when_caller_forced():
    ...
    force=(stale_reload or bool(force)). A stale-reload is gated now (the 1h
    backstop catches genuine breakage); only the manual Update button (force=True)
    bypasses the gate."""
    ...
    assert "force=(stale_reload or bool(force))" not in _SRC_TEXT, (
        ...
        "users mid-day; stale-reload is gated + 1h backstop now")
