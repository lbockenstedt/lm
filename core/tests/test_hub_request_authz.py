"""H1: gate the reverse HUB_REQUEST channel to a pinned AppBuilder client cert.
...
(``GET_SPOKE_STATUS``). It's AppBuilder's tool — but nothing restricted it to
AppBuilder, so a malicious tenant-added spoke (or a compromised box) escalated to
fleet-wide action.
...
specific Let's Encrypt-issued cert as "the AppBuilder cert" (the LE-module
checkbox → ``global_config['ab_cert_identities']``, a list of DNS names);
...
Rule: ``AppBuilder rights ⟺ mTLS on AND the connection's verified client cert
matches the pinned AppBuilder cert. Anything else → denied.``
...
def test_pinned_cert_match_authorizes():
    """The happy path: a pinned AppBuilder cert presented over mTLS authorizes the
    channel. ``peer_cert_identity`` is the tuple of SAN DNS names extracted from
    the verified client cert."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    assert h._hub_request_authorized("ab", ("ab.lm.io",)) is True


def test_no_pinned_cert_denies_even_for_ab_spoke_id():
    """Fail-closed default: with no cert designated as AppBuilder, the channel is
    CLOSED — even a spoke literally named 'ab' is denied. AppBuilder is
    dormant until the operator issues + labels a dedicated cert."""
    h = _hub()
    assert h._hub_request_authorized("ab", ("ab.lm.io",)) is False
    # An empty list is the same as absent.
    h2 = _hub({"ab_cert_identities": []})
    assert h2._hub_request_authorized("ab", ("ab.lm.io",)) is False


def test_no_peer_cert_denies():
    """No client cert presented (mTLS off / plaintext / extraction failed) →
    denied, even when a AppBuilder cert is pinned. The spoke_id alone is NOT a
    credential."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    assert h._hub_request_authorized("ab", None) is False
    assert h._hub_request_authorized("ab", ()) is False
    # A spoke that merely claims the ab id but presents no cert is denied.
    assert h._hub_request_authorized("ab", None) is False


def test_cert_mismatch_denies():
    """A valid fleet cert that is NOT the pinned AppBuilder cert is denied — every
    spoke presents the same LE wildcard, so chain-validity alone can't distinguish
    AppBuilder. Only the pinned identity authorizes."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    assert h._hub_request_authorized("netbox-server", ("*.lm.io",)) is False
    assert h._hub_request_authorized("ab", ("hub.lm.io",)) is False


def test_spoke_id_alone_never_authorizes():
    """The hostname-spoof hole this closes: a box named 'ab' (or
    'x-ab-y') presenting the generic fleet cert is NOT authorized — identity
    is the cert, not the spoke_id claim. spoke_id is ignored entirely."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    for sid in ("ab", "AB", "AppBuilder", "x-ab-y", "my-ab-1"):
...
def test_any_pinned_name_matching_multi_san_cert_authorizes():
    ...
    any pinned name) authorizes. Covers a AppBuilder cert issued with both an apex
    and a wildcard / alias SAN."""
    h = _hub({"ab_cert_identities": ["ab.lm.io", "fixer.lm.io"]})
    assert h._hub_request_authorized("ab", ("ab.lm.io", "fixer.lm.io")) is True
    # Match on the second pinned name alone.
    assert h._hub_request_authorized("ab", ("fixer.lm.io", "other.lm.io")) is True
...
def test_pinned_match_is_case_sensitive():
    ...
    cert presenting 'AppBuilder.LM.IO' does not match a pinned 'ab.lm.io'."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    assert h._hub_request_authorized("ab", ("AppBuilder.LM.IO",)) is False
    assert h._hub_request_authorized("ab", ("ab.lm.io",)) is True
...
def test_handle_hub_request_denies_non_authorized_and_records_event():
    """A spoke presenting the generic fleet cert (not the pinned AppBuilder cert)
...
def test_handle_hub_request_denies_get_logs_without_pinned_cert():
    ...
    cert, because that cert isn't the pinned AppBuilder cert."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    r = asyncio.run(
...
def test_handle_hub_request_denies_when_no_cert_presented():
    ...
    'ab'. The cert is the credential, not the spoke_id."""
    h = _hub({"ab_cert_identities": ["ab.lm.io"]})
    r = asyncio.run(
        h.handle_hub_request("ab", {"type": "GET_LOGS"}, None))
    assert r["status"] == "error"
    assert "not authorized" in r["message"]


def test_handle_hub_request_allows_pinned_ab_get_logs_to_proceed():
    """AppBuilder presenting the pinned cert still gets through to the GET_LOGS
    ...
    h = _hub({"ab_cert_identities": ["ab.lm.io"]},
             extra={"collect_all_logs": lambda: {"logs": {"ab": ["line1"]}}})
    r = asyncio.run(
        h.handle_hub_request("ab", {"type": "GET_LOGS"},
                             ("ab.lm.io",)))
    assert r == {"logs": {"ab": ["line1"]}}
