...
class SpokeAlertMixin:
    ...
    async def run_spoke_alert_loop(self):
        ...
        """
        def _guard() -> bool:
            if not self._spoke_alert_cfg().get("enabled", False):
        ...
        def _delay() -> float:
            enabled = self._spoke_alert_cfg().get("enabled", False)
            return self._SPOKE_ALERT_LOOP_S if enabled else 60

        # stagger 30s: let spokes connect before first evaluation
        await run_sync_loop(
            stagger=30, guard=_guard, body=_body, delay=_delay,
            on_error=lambda e: logger.warning("[spoke-alert] loop cycle failed: %s", e))