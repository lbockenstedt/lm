"""Coordinator-role instance pool failover for the Hub.
...
PR2 of a three-part design (see project memory
``coordinator-role-pool-and-failover-design.md``). PR1 (routes/role_pool.py)
let an operator bind a coordinator-role instance (NAC/ClearPass, IPAM/NetBox,
LDAP/directory) to a spoke and have LOAD_ROLE fire automatically. This PR adds
an optional ``spoke_pool`` (a checkbox-selected, unordered list of candidate
base-agent ids) to an instance record: when the CURRENTLY active spoke
(``spoke_id``, resolved by PR1 to a role sub-spoke id) goes out of contact,
this loop relocates the instance to another connected pool candidate — cold
switch, single-writer at all times (never splits/shares one instance across
multiple spokes), and deliberately **no auto-failback**: once relocated, the
instance simply stays on its new spoke; a returned original node is just an
ordinary candidate again for a FUTURE relocation, never automatically
reclaimed. A record with no ``spoke_pool`` (or an empty one) is untouched by
this loop — that's the plain PR1 single-spoke behavior, unchanged.

A nice emergent property worth calling out: relocation tries pool candidates
in stored order, INCLUDING the currently-active one's own base agent. If only
that spoke's role *sub*-connection died (the role process crashed) while its
base-agent connection is still alive, this self-heals in place — LOAD_ROLE
just restarts the role on the same box — rather than needlessly moving to a
different one. Only a base agent that's actually unreachable gets skipped.
...
class InstanceRelocateMixin:
    ...
    async def _relocate_instance(self, inst, storage_key, role, module_type, pool) -> None:
        ...
        logger.warning("[instance-relocate] %s '%s': relocated from %s to %s",
                       storage_key, name, old_active, new_sub_id)
        try:
    ...
    async def _relocate_unload_orphan(self, old_sub_id, role, storage_key, excluding_id) -> None:
        ...
        if still_used:
            return
        suffix = f"-{role}"
        base_id = old_sub_id[:-len(suffix)] if old_sub_id.endswith(suffix) else old_sub_id
