...
#   --onboarding-psk <psk>    Self-service tenant onboarding: presented in the
#   --tenant-hint <tenant>    WS connect handshake — on a match against that
#                  tenant's stored PSKs (generated by a tenant-admin via
#                  Add Server, /tenant/{tenant}/onboarding-psk — no
#                  Global-Admin /setup/* approval needed), this spoke
#                  auto-approves + auto-binds to that tenant immediately.
#                  Both required together; omit for the normal
...
# --onboarding-psk + --tenant-hint: a tenant-admin's self-service "Add
# Server" onboarding (Setup UI is Global-Admin-only; a tenant-admin
# generates their own PSK via /tenant/{tenant}/onboarding-psk instead). The
# control-plane base class already reads LM_ONBOARDING_PSK/LM_TENANT_ID_HINT
# from the environment and presents them in the WS connect handshake
# (control_plane.py) — main.py._try_psk_self_provision validates the PSK
# against the hinted tenant's stored PSKs and, on match, auto-approves +
# auto-binds this spoke to that tenant with NO admin click required. Falls
# through to the normal pending-admin-approval path on any mismatch/absence
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --tenant-hint)     TENANT_ID_HINT="$2"; shift ;;
...
LM_ONBOARDING_PSK=$ONBOARDING_PSK
LM_TENANT_ID_HINT=$TENANT_ID_HINT
${INSTALL_UUID_LINE}
EOF
